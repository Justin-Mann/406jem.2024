...
- **CORS:** Required for the cookie-based cross-client session (#47) but not yet configured — see "Cross-client cookie session (#47)" under the Auth section below for the exact required Azure Function App CORS setting.
...
- **`Auth:CookieDomain` app setting (new):** must be set to `406jem.com` on the **production** slot only — a `Domain=406jem.com` attribute is invalid (and silently rejected by the browser) on a response from any other host, including the `staging` slot and local dev. Leave unset on staging/local, which yields a host-only cookie.
...
- **CORS is now a required manual Azure Functions app setting, not code:** Azure Functions' CORS (including `Access-Control-Allow-Credentials`, needed for a cookie-based session) is a platform-level Function App setting (Portal → CORS blade, or `az functionapp cors add`/`az functionapp cors credentials`) — isolated-worker app code can't implement it itself, since preflight `OPTIONS` requests are answered by the Functions host before they ever reach the worker process. This is **not yet configured as part of #47** and is a deployment blocker for the feature to work cross-origin in production: the production Function App's CORS allowed-origins list must include `https://406jem.com` and `https://angular.406jem.com` (plus local dev origins, e.g. `http://localhost:4200`, `http://localhost:5000`, `https://localhost:7154`, for anyone testing against a deployed API from a local frontend) with "Access-Control-Allow-Credentials" enabled.
