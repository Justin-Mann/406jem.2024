...
public class ResumePosterContactApiTests
{
    ...
    private readonly ResumePosterContactApi _api;

    public ResumePosterContactApiTests()
    {
        _rateLimitStore.TryRecordAttemptAsync(Arg.Any<string>()).Returns(true);
        _api = new ResumePosterContactApi(Substitute.For<ILogger<ResumePosterContactApi>>(), _userStore, _emailSender, _rateLimitStore);
    }
